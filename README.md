# AI Contour — Система автооконтурирования органов риска (OAR)
...
1. **Клиентская часть (`client_app.py`):**
   * Легковесное PyQt5-приложение (с поддержкой API PyQt6 через шим) с интуитивно понятным интерфейсом.
   ...
   * **Компактная индикация:** Актуальный статус соединения с сервером отображается пиктограммами (`❌`, `✅`, `⏸️`) в верхнем углу вкладок без лишней нагрузки на интерфейс.
...
2. **Серверная часть (`server_app.py` + `server/`):**
   ...
   * **Управление очередью:** Оператор сервера может на лету переупорядочивать очередь PENDING задач с помощью удобного Drag & Drop механизма перетаскивания строк мышью.
...
* **Пароль администратора:** Для изменения адреса подключения к серверу, управления лицензиями суб-моделей или просмотра подключенных клиентов, нажмите кнопку **«⚙️ Настройки администратора»** во вкладке «Настройки» и введите пароль:
  ```text
  rtp
  ```
* **Безопасность и Черный список (IP-Dynamic DHCP Safety)**:
  * Система поддерживает блокировку нежелательных ПК. Вместо ненадежной привязки к IP-адресам (динамический DHCP в больничных сетях) блокировка производится по уникальному текстовому **Client ID** (имя ПК), который передается в заголовке `X-Client-ID`.
  * Администратор сервера может управлять черным списком в реальном времени через диалоговое окно «👥 Подключенные клиенты» в панели настроек.
...
В проекте реализована система автоматических юнит-тестов, расположенных в директории `tests/`. Тесты проверяют стабильность расчетов, корректность сетевых запросов и обработку отмены задач:
* **`test_styles.py`** — проверка QSS-стилей оформления.
* **`test_network_worker.py`** — тестирование сетевого взаимодействия клиента и сервера.
* **`test_cancel_job.py`** — проверка корректности прерывания активных фоновых процессов ИИ при отмене задачи.
* **`test_memory_leak.py`** — стресс-тест на отсутствие утечек оперативной памяти в контур-движке.

Для запуска всего набора тестов выполните в виртуальном окружении:
```powershell
